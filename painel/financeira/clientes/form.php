<?php
    ...
    if($_POST['acao'] == 'salvar'){
        ...
        if($_POST['codigo']){
            ...
            mysqli_query($con, "insert into clientes_origem set cliente = '{$cod}', origem = 'sistema', data = NOW()");
        }
        ...
        mysqli_query($con, "update consultas_log set ativo = '0' where cliente = '{$_POST['codigo']}'");
        ...
        mysqli_query($con, "update clientes set ultimo_acesso = NOW(), status_atual = '".addslashes(json_encode($status))."' where codigo = '{$_POST['codigo']}'");
    ...
    <form id="form-<?= $md5 ?>">
        ...
        <div class="row g-0">
            <div class="col">
                ...
                <div class="form-floating mb-3">
                    <select required name="tipo_midia" id="tipo_midia" class="form-select">
                        <option value="">:: Selecione a mídia ::</option>
                        <?php
                        $q = "select * from tipo_midia order by nome";
                        $r = mysqli_query($con,$q);
                        while($s = mysqli_fetch_object($r)){
                        ?>
                        <option value="<?=$s->codigo?>" <?=(($d->tipo_midia == $s->codigo)?'selected':false)?>><?=$s->nome?></option>
                        <?php
                        }
                        ?>
                    </select>   
                ...
                <div class="mb-3">
                    <label>Bancos Autorizados*</label>
                        <?php
                        $bancos_autorizados = json_decode($d->bancos_autorizados);
                        $checked = [];
                        foreach($bancos_autorizados as $ind => $val){
                            $checked[] = $val->codigo;
                        }
                        $q = "select * from bancos order by banco";
                        $r = mysqli_query($con,$q);
                        while($s = mysqli_fetch_object($r)){
                        ?>
                        <div class="form-check">
                            <input 
                                type="checkbox" 
                                class="form-check-input bancos" 
                                id="banco<?=$s->codigo?>" 
                                value="<?=$s->codigo?>" 
                                banco="<?=$s->banco?>"
                                <?=((in_array($s->codigo,$checked))?'checked':false)?>
                            >
                            <label class="form-check-label" for="banco<?=$s->codigo?>"><?=$s->banco?></label>
                        </div>
                        <?php
                        }
                        ?>
                ...
                <h6 style="margin-top:50px;">Cadastro Completo</h6>
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="birthdate" id="birthdate" class="form-control" placeholder="Data de Nascimento" value="<?=dataBr($d->birthdate)?>">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="nationality" id="nationality" class="form-control" placeholder="Nacionalidade" value="<?=$d->nationality?>">
                    <label for="nationality">Nacionalidade</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="naturalness" id="naturalness" class="form-control" placeholder="Naturalidade" value="<?=$d->naturalness?>">
                    <label for="naturalness">Naturalidade</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="motherName" id="motherName" class="form-control" placeholder="Nome da Mãe" value="<?=$d->motherName?>">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="document_number" id="document_number" class="form-control" placeholder="Número do Documento" value="<?=$d->document_number?>">
                    <label for="document_number">Número do Documento</label>
                </div>


                <div class="form-floating mb-3">
                    <select requiredX name="document_issuingState" id="document_issuingState" class="form-select">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="document_issuingAuthority" id="document_issuingAuthority" class="form-control" placeholder="Orgão Emissor" value="<?=$d->document_issuingAuthority?>">
                    <label for="document_issuingAuthority">Orgão Emissor</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="document_issueDate" id="document_issueDate" class="form-control" placeholder="Data da Emissão" value="<?=dataBr(trim($d->document_issueDate))?>">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="address_zipCode" id="address_zipCode" class="form-control" placeholder="CEP" value="<?=$d->address_zipCode?>">
                    <label for="address_zipCode">CEP</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="address_street" id="address_street" class="form-control" placeholder="Avenida, rua ou Beco" value="<?=$d->address_street?>">
                    <label for="address_street">Logradouro</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="address_number" id="address_number" class="form-control" placeholder="Número da Moradia" value="<?=$d->address_number?>">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="address_city" id="address_city" class="form-control" placeholder="Cidade" value="<?=$d->address_city?>">
                    <label for="address_city">Cidade</label>
                </div>


                <div class="form-floating mb-3">
                    <select requiredX name="address_state" id="address_state" class="form-select">
                        <option value="">:: Selecione o Estado ::</option>
                        <?php
                        foreach($siglas as $i => $sigla){
                        ?>
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="renda" id="renda" class="form-control" placeholder="Cidade" value="<?=$d->renda?>">
                    <label for="renda">Renda</label>
                </div>

                <div class="form-floating mb-3">
                    <input requiredX type="text" name="valor_patrimonio" id="valor_patrimonio" class="form-control" placeholder="Cidade" value="<?=$d->valor_patrimonio?>">
                ...
                <div class="form-floating mb-3">
                    <select requiredX name="bankCode" id="bankCode" class="form-select">
                ...
                <div class="form-floating mb-3">
                    <input requiredX type="text" name="accountNumber" id="accountNumber" class="form-control" placeholder="Número da Conta" value="<?=$d->accountNumber?>">
                    <label for="accountNumber">Número da Conta</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="accountDigit" id="accountDigit" class="form-control" placeholder="Dígito da Conta" value="<?=$d->accountDigit?>">
                    <label for="accountDigit">Dígito da Conta</label>
                </div>


                <div class="form-floating mb-3">
                    <input requiredX type="text" name="branchNumber" id="branchNumber" class="form-control" placeholder="Agência" value="<?=$d->branchNumber?>">
        ...
        <div class="row g-0" id="acao-<?=$md5?>">
            <div class="col">
                <div style="display:flex; justify-content:end;">
                    <button type="submit" class="btn btn-success btn-ms mt-1">Salvar</button>
            ...
            $('#form-<?=$md5?>').submit(function (e) {
                ...
                $(".bancos").each(function(){
                    if($(this).prop("checked")){
                        bancos.push({'codigo':$(this).val(), "banco":$(this).attr("banco")})
                    }
                })
                ...
                if(!bancos.length){
                    ...
                    campos.push({name: 'bancos_autorizados', value: JSON.stringify(bancos)})
                }
