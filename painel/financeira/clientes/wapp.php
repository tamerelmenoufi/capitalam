...
<h4 class="Titulo<?=$md5?>">
    <div class="d-flex justify-content-between align-items-center">
        ...
        ?>
        <i voltar class="fa-solid fa-circle-arrow-left" style="cursor:pointer"></i>
        <?php
        }
        ?>
...
<div chatWindow="open" class="palco<?=$md5?>" up<?=$phoneNumber?>>
    <?php
    if($phoneNumber){
        $query = "select * from wapp_chat where (de = '{$ConfWappNumero}' and para LIKE '%{$phoneNumber}' or de LIKE '%{$phoneNumber}' and para = '{$ConfWappNumero}') order by data asc";
        $result = mysqli_query($con, $query);
        $update = [];
    ...
    <?php
        ...
        if($update){
            $q = "update wapp_chat set recebida = '1' where codigo in(".implode(', ', $update).") and recebida != '1'";
            mysqli_query($con, $q);
        }
