<?php
        ...
        if($_POST['cor']){
            ...
            mysqli_query($con, "update status set cor = '{$_POST['cor']}' where codigo = '{$_POST['cod']}'");
...
<div class="d-flex flex-row-reverse">
    ...
    <button telefones type="button" class="btn btn-warning btn-sm ms-3"><i class="fa-solid fa-gears"></i> Telefones</button>
    <div>
        <div class="input-group">
            <input type="color" cor class="form-control form-control-sm" style="min-width:50px;" value="<?=(($d->cor)?:'#000000')?>" />
            <button class="btn btn-danger salvarCor btn-sm" type="button">Salvar Cor</button>
        </div>
    </div>
</div>
<?php
        ...
        $(".salvarCor").click(function(){
            ...
            $.ajax({
                ...
                success:function(dados){
                    ...
                    // openedCanvas.hide();
                    Carregando('none');
                    $.alert(`Cor <span style='color:${cor}; background-color:${cor}'>${cor}</span> atualizada com sucesso!`);
                },
                error:function(){
