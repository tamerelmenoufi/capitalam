<?php
        ...
        if($_POST['acao'] == 'salvar'){
            ...
            if($_POST['base64_arq'] and $_POST['imagem_tipo_arq'] and $_POST['imagem_nome_arq']){
              ...
              if(is_file("../../volume/wapp/status_geral/{$_POST['status']}/{$_POST['imagem_arq']}")) unlink("../../volume/wapp/status_geral/{$_POST['status']}/{$_POST['imagem_arq']}");
              ...
              if(!is_dir("../../volume/wapp/status_geral")) mkdir("../../volume/wapp/status_geral");
              if(!is_dir("../../volume/wapp/status_geral/{$_POST['status']}")) mkdir("../../volume/wapp/status_geral/{$_POST['status']}");
              if(file_put_contents("../../volume/wapp/status_geral/{$_POST['status']}/".$nome, $img)){
                $dados['arquivo'] = $nome;
              }
...
<form>
    ...
    <div showImage_arq class="form-floating" style="display:<?=(($m->arquivo)?'block':'none')?>">
        <img src="volume/wapp/status_geral/<?="{$d->codigo}/{$m->arquivo}"?>" class="img-fluid mt-3 mb-3" alt="" />
    </div>
