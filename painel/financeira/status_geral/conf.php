<?php
        ...
        if($_POST['excluir']){
            ...
            if(is_file("../../volume/wapp/status_geral/{$e->status}/{$e->arquivo}")) unlink("../../volume/wapp/status_geral/{$e->status}/{$e->arquivo}");
        ...
        $query = "select * from status_geral where codigo = '{$_POST['cod']}'";
...
<h5><?="{$d->codigo} - {$d->descricao}"?></h5>

<div class="d-flex flex-row-reverse">
    <button novo type="button" class="btn btn-success btn-sm ms-3"><i class="fa-solid fa-comment-medical"></i> Novo</button>
    <button telefones type="button" class="btn btn-warning btn-sm"><i class="fa-solid fa-gears"></i> Telefones xxx</button>
    <input type="color" cor class="form-control form-control-sm" />
</div>
<?php
...
<div class="card mt-3">
  ...
  <div class="card-body">
        ...
        ?>
        <audio controls>
        <source src="volume/wapp/status_geral/<?="{$d->codigo}/{$m->arquivo}"?>" type="audio/ogg">
        <source src="volume/wapp/status_geral/<?="{$d->codigo}/{$m->arquivo}"?>" type="audio/mpeg">
        ...
        <div class="alert alert-primary d-flex align-items-center" role="alert">
            <a style="margin-right:10px;" href="volume/wapp/status_geral/<?="{$d->codigo}/{$m->arquivo}"?>" target="_blank">
        ...
        <?php  
                ...
                echo "<div class='d-flex justify-content-center'><img class='img-fluid' src='volume/wapp/status_geral/{$d->codigo}/{$m->arquivo}' /></div>";
            }
            if($m->mensagem){
                echo "<p>{$m->mensagem}</p>";
            }
...
<script>
    $(function(){
        ...
        $("button[novo]").click(function(){
            $.ajax({
                url:"financeira/status_geral/conf_form.php",
                type:"POST",
                data:{
                    cod:'<?=$d->codigo?>'
                },
        ...
        $("button[telefones]").click(function(){
            $.ajax({
                url:"financeira/status_geral/conf_numeros.php",
                type:"POST",
                data:{
                    cod:'<?=$d->codigo?>'
                },
        ...
        $("button[enviar]").click(function(){
            ...
            $.ajax({
                url:"financeira/status_geral/enviarWapp.php",
                type:"POST",
                data:{
                    envio
                },
